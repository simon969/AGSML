 ...
 * @author thomsonsj
 */
class OSNationalGridConversion {
    public double Easting;
    public double Northing;
    public double Latitude;
    public double Longitude;
    public BigDecimal bdLatitude;
    public BigDecimal bdLongitude;
    public enumConversionType conversionType ;
    
    public enum enumConversionType {
        BigDecimal, Float, Double
    }
public OSNationalGridConversion() {
    conversionType=enumConversionType.Double;
} 
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
private boolean OSGridToLatLong_BigDecimal() {
////  var gr = gridrefLetToNum(gridRef);
////  var E = gr[0], N = gr[1];
//try {
//    
//   BigDecimal a = new BigDecimal(6377563.396);
//   BigDecimal b = new BigDecimal(6356256.910);              // Airy 1830 major & minor semi-axes
//   BigDecimal F0 = new BigDecimal(0.9996012717);                             // NatGrid scale factor on central meridian
//   BigDecimal lat0 = 49*Math.PI/180, lon0 = -2*Math.PI/180;  // NatGrid true origin
//   BigDecimal N0 = -100000, E0 = 400000;                     // northing & easting of true origin, metres
//   BigDecimal e2 = 1 - (b*b)/(a*a);                          // eccentricity squared
//   BigDecimal n = (a-b)/(a+b), n2 = n*n, n3 = n*n*n;
//
//  BigDecimal lat=lat0, M=0;
//  do {
...
//  
//  bdLatitude =  lat /(Math.PI) * 180);
//  bdLongitude = Math.toDegrees(lon);
//  
//  return true;
//  
//} catch (Exception e) {
...
private boolean OSGridToLatLong_Double() {
//  var gr = gridrefLetToNum(gridRef);
//  var E = gr[0], N = gr[1];
try {
...
public boolean LatLongToOSGrid() {

    return true;
}
public String dbOSGridToLatitudeLongitudePair() {
    
    OSGridToLatLong_Double();
    
    return Double.toString(Latitude) + ','  + Double.toString(Longitude);
} 
public String dbOSGridToLongitudeLatitudePair() {
    
    OSGridToLatLong_Double();
    
    return Double.toString(Longitude) + ','  + Double.toString(Latitude);
} 
public String dbLatLongToEastingNorthingPair() {
